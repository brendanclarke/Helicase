...
/*
 * Drum descriptor source of truth.
 *
 * Each row binds one stable on-card key to one explicit local ID and storage
 * owner. Defaults/ranges are byte-domain values; Menu labels and precise target
 * flags are filled from the existing UI tables during the Menu migration.
 * Clients: InstrumentManager lookup, Scene reset, storage parsing, Menu, and
 * routing validation.
 */
...
const ParamDescriptor drum_param_descriptors[] = {
    ...
    IMAGE("osc2_wave", INST_PARAM_OSC2_WAVE),
    IMAGE("osc2_pitch_coarse", INST_PARAM_OSC2_PITCH_COARSE),
    IMAGE("osc2_mod_amount", INST_PARAM_OSC2_MOD_AMOUNT),
    IMAGE("osc2_mod_type", INST_PARAM_OSC2_MOD_TYPE),
