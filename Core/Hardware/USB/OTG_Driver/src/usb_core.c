...
USB_OTG_STS USB_OTG_CoreInit(USB_OTG_CORE_HANDLE *pdev)
{
  ...
  if (pdev->cfg.phy_itface == USB_OTG_ULPI_PHY)
  {
    ...
    gccfg.b.pwdn = 0;
    /* sofouten removed — not a valid field on F7 GCCFG (ULPI path unused) */
  ...
  else /* FS interface (embedded Phy) */
  {
    ...
    **
    ** F7 GCCFG changes vs F4:
    **   pwdn    (bit 16): same — 1 = transceiver powered up
    **   vbden   (bit 22): VBUS detection enable. We set 0 because the
    **                     ADUM3160 USB isolator on the LXR-02 handles
    **                     the VBUS connection — the STM32 does not need
    **                     to detect it. Replaces F4's vbussensingA/B and
    **                     disablevbussensing bits.
    **   F4's sofouten (bit 20) is now pden (primary detection) on F7
    **   and must NOT be set here — SOF output not used.
    */
    gccfg.d32 = 0;
    gccfg.b.pwdn = 1;   /* power up transceiver */
    /* vbden = 0: VBUS detection disabled (ADUM3160 handles isolation) */
    USB_OTG_WRITE_REG32 (&pdev->regs.GREGS->GCCFG, gccfg.d32);
    USB_OTG_BSP_mDelay(20);

    /* Force B-peripheral session valid.
    **
    ** The ADUM3160 USB isolator prevents the STM32 from seeing VBUS from
    ** the host, so the OTG controller's VBUS detection never fires and it
    ** never asserts the D+ pull-up. GOTGCTL.BVALOEN + BVALOVAL tell the
    ** controller to treat the B-session as valid regardless of VBUS, which
    ** causes it to enter device mode and assert the pull-up so the host
    ** detects the device. This is the standard fix for USB behind an
    ** isolator on STM32F7 (not needed on F4 which had different GCCFG). */
    {
      USB_OTG_GOTGCTL_TypeDef gotgctl;
      gotgctl.d32 = USB_OTG_READ_REG32(&pdev->regs.GREGS->GOTGCTL);
      gotgctl.b.bvaloen  = 1;   /* B-valid override enable */
      gotgctl.b.bvaloval = 1;   /* B-valid override value = valid */
