...
uint32_t sesreq :
    1;
/* F7-specific override bits (Reserved2_7 on F4) */
uint32_t vbvaloven :
    1;  /* bit 2: A/VBUS valid override enable */
uint32_t vbvaloval :
    1;  /* bit 3: A/VBUS valid override value */
uint32_t avaloen :
    1;  /* bit 4: A-valid override enable */
uint32_t avaloval :
    1;  /* bit 5: A-valid override value */
uint32_t bvaloen :
    1;  /* bit 6: B-valid override enable  */
uint32_t bvaloval :
    1;  /* bit 7: B-valid override value   */
uint32_t hstnegscs :
    1;
uint32_t hnpreq :
    1;
...
typedef union _USB_OTG_GCCFG_TypeDef
{
  ...
  struct
  {
    /* STM32F765 OTG_FS_GCCFG register layout (RM0410 section 34.14.2)
    ** Replaces the F4 layout which had vbussensingA/B and disablevbussensing.
    ** F7 moves VBUS detection to a single VBDEN bit and adds battery
    ** charging detection fields in the middle. */
    uint32_t Reserved_in : 16;  /* bits 15:0  reserved              */
    uint32_t pwdn        :  1;  /* bit  16    power down (1=powered) */
    uint32_t Reserved_17 :  1;  /* bit  17    reserved               */
    uint32_t bcden       :  1;  /* bit  18    battery charging det.  */
    uint32_t dcden       :  1;  /* bit  19    data contact det.      */
    uint32_t pden        :  1;  /* bit  20    primary detection      */
    uint32_t sden        :  1;  /* bit  21    secondary detection    */
    uint32_t vbden       :  1;  /* bit  22    VBUS detection enable  */
    uint32_t Reserved_out:  9;  /* bits 31:23 reserved               */
  }
  b;
