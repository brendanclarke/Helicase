...
`autosave_getLivePayloadByte()` now maps explicit BankData, Scene settings, Kit
settings, Instrument type text, normal endpoints, and Morphable Morph endpoints.
It rejects names, padding, absent Scenes, Effects, Pattern, unknown types,
out-of-range descriptors, non-Morphable Morph cells, and
...
3. captures at most 256 successful live gets per transaction;
...
- 3,856 mask bytes;
- 256 `uint16_t` payload offsets; and
- 256 captured byte values.

Its linked size is exactly 4,624 bytes (`0x1210`), below the 9 KB ceiling. The
...
BSS bytes; the completed build uses 74,604, an exact 4,624-byte increase.
