...
- Fixed Bank Save child iteration after hardware retest:
  ...
  - Follow-up `SD_CARD/Bank/000 SlakBad2/` showed fresh Bank metadata for Scene 01 but stale/missing Scene 01 payload files. Root Scene Load still opened the selected Scene by cached SFN alias and scanned children with the older raw finder/LFN side state. It now opens the visible `NNN Name` Scene folder with `afatfs_opendir_lfn()` and scans child files/directories with `afatfs_findNextObject()`, so loading root `005 Slak2` into a resident Scene slot should stage the actual `Kit Forest`, `sceneset.scg`, `pattern.pat`, and `effects.fx` payload before Save Bank serializes that slot.
  - Follow-up `SD_CARD/Bank/000 SlakBad3/` proved the overwrite path was still merging into a dirty Bank tree: `00 Slak/Kit Slak` contained both old compact Instrument filenames and the newly renamed variants. Bank Save no longer mutates the numbered Bank folder in place and no longer relies on recursive delete in the successful overwrite path. It writes a complete non-numbered temp folder such as `tmp000-4a3f`, renames the previous `NNN Name` Bank to a paired non-numbered `old000-4a3f` folder, then renames the temp folder to the final numbered slot. The scratch suffix mixes hardware RNG, the 1ms tick, and a save counter because asyncfatfs mkdir is create-or-open; this makes reopening a stale temp component across repeated retests much less likely. Bank scans ignore `tmp...` and `old...` because they do not parse as `NNN Name`; these displaced folders are intentionally left for manual/later cleanup instead of risking another async recursive-delete hang during Save Bank.
  - Temp promotion failure is now explicit: asyncfatfs rename completion is latched separately, and the returned open-name buffer must be non-empty after temp-to-final promotion. If the old numbered Bank did not exist, Save Bank continues; if the temp tree cannot become the numbered slot, Save Bank reports `BProm2D` and does not pretend the slot was safely published.
  - Follow-up `SD_CARD/Bank/000 SlakBad4/` showed Save Bank could still reopen a stale temp folder: Scene 01 contained both the correct loaded `Kit Forest` and the previous `Kit Slak`, while other children still had duplicate Instrument filename generations. Bank Save now preflight-scans `/Bank` before temp creation and accepts a scratch pair only when neither `tmp...` nor paired `old...` currently exists. If either name is present, it generates a new pair and rescans; after 16 collisions it reports `BTmp30` instead of opening a stale temp directory.
