...
Session 035 completed the next Phase 3 filesystem/runtime pass. Kit Morph Load parses normal Kit directories and copies source normal endpoint values into resident morph endpoints only for same-type slots; Instrument Morph Load is available only for the destination slot's current type and applies the same no-change mismatch rule. Instrument files now support storage-only LFO target voice `self`, resolved on load to the destination slot and emitted on Kit Save when an LFO voice target points at the saved instrument's own slot. Descriptor LFO modulation now uses descriptor-owned parameter-domain metadata plus InstrumentManager adapters, so temporary values go through the normal owner runtime writer and negative polarity matches original LXR value-relative math. Normal `Save:[Kit     ]` now writes new-format `Kit/<NNNxxxxx>/kitset.kcg` plus six instrument files with `[params]` and `[morph]`; Kit slots are 001..999, SceneData retains 16-character instrument save stems, and storage/file writing avoids stdio so the embedded link stays clean. No asyncfatfs core API was added; future save work should reuse existing `afatfs_mkdir`, `afatfs_fopen("w")`, `afatfs_fclose`, and `afatfs_chdir` through filesystem state machines.
...
| Normal `Save:[Kit     ]` writes directory Kit format: `Kit/<NNNxxxxx>/kitset.kcg` plus six instrument files with `[params]` and `[morph]`; Kit slots are 001..999 / uint16_t | 035 |
| asyncfatfs core was not expanded for Kit Save; future save code should reuse `afatfs_mkdir`, `afatfs_fopen(..., "w", ...)`, `afatfs_fclose`, and `afatfs_chdir`; LFN creation, rename/replace, and recursive directory replace remain missing primitives | 035 |
