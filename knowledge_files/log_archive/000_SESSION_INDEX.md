...
| 030 | 2026-07-07 | local working directory, branch `dev-burst-reduction` | Phase 2 filesystem start: `FILESYSTEM_SPEC.md`, generated `SD_CARD/Kit`, `storageTypes.c/h`, directory-kit load from root `Kit/`, space/underscore numbered folder scan |
...
| Instrument file schema, Scene descriptor storage, dynamic VOICE pages, and DSP propagation are specified in `knowledge_files/specification_reference/INSTRUMENT_FILE_SPEC.md` | 032 |
...
| Descriptor Morph is known broken on hardware after Session 032 | 032 |
| LFO/velocity target assignment storage/display exists for descriptor parameters, but runtime modulation and step automation still need descriptor-aware `ModulationNode`/`AutomationNode` work | 032 |
| Instrument parser keys must allow at least `amp_envelope_decay_closed/open` length; Session 032 uses `STORAGE_INSTRUMENT_KEY_MAX=32` | 032 |
...
| Phase 2 root SD layout is documented in `FILESYSTEM_SPEC.md`: `Bank`, `Scene`, `Kit`, `Pattern`, `Sample`, `Wavetable`, `Effect`, `Instrument`, and root `settings.cfg` | 030 |
...
| LED flash is a group overlay with 16-bit masks, per-group cancel/restore, and retained 400/80 timing; final LED layer consolidation is SCOPING Phase 3.11 | 031 |
