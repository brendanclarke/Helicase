...
| 068 | 2026-09-19 | commit `613f466` on `dev-ph4-5-fixes` (base `9989811`) | Front-panel event ring rebuilt as a 64-entry monotonic SPSC ring (was 16-entry masked head/tail, 15 usable, silent overflow) with overflow reconciliation (clears stale VOICE-Scene/Load-Scene pairing masks, cancels the shared hold timer), a physical-held check before hold-timer promotion, a second bounded `buttonHandler_processEvents()` drain per main-loop pass, a `DEV_MODE_LOGGING` step-toggle witness (`K`) immediately before `pat_toggleStep()`, and `BUTTON_HOLD_DELAY_MS` 100→200ms — fixes silently-dropped VOICE-mode step-toggle taps, root-caused to ring overflow during multi-button release bursts; missing chase-light fix: new `menu_setPlayedPattern()` called at all three filesystem Scene/Bank realignment sites so the played-Pattern UI mirror is never left at its stale BSS default after boot; per-track sequencer step length now read from `pat_scene_region_t.track_length` in `seq_advanceTrackStep()` and `seq_realignActivePatternToMasterClock()` (was hardcoded `NUM_STEPS_PER_BAR`=16), default init changed 128→16, 35 SD-card PAT4 files patched; all three hardware-accepted. Per-track scale/shuffle sequencer consumption and the AutoSave/Pattern-service bounded-CPU convergence plan are deferred to Session 069 |
