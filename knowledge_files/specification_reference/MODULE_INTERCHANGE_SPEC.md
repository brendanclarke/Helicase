...
| `filesystem_requestLoadResidentKitName()` | Borrow HCNAMES for one Scene's Kit-plus-six block during Kit-family browsing. Normal Kit Load/Save publishes its seven affected rows from the filesystem operation before callback. | combined Kit/Instrument Menu read session and filesystem Kit Load/Save |
| `filesystem_requestLoadResidentInstrumentName()` | Borrow one Instrument HCNAMES row during nested browsing. Normal Instrument Load/Save publishes its affected row from the filesystem operation before callback. | Menu read session and filesystem Instrument Load/Save |
| `filesystem_requestLoadResidentSceneName()` | Borrow one Scene HCNAMES row during Scene-menu browsing. Root Scene Load/Save publishes Scene plus embedded Kit/Instrument rows from the filesystem operation before callback. | Scene-menu read session and filesystem Scene Load/Save |
