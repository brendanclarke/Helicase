...
**Address array.** Every scene has 7 tracks × 128 steps = 896 possible steps, each represented by a fixed `896 × 2 bytes = 1,792 bytes` per-scene array. Each 16-bit entry is **bit 15: on/off, bit 14: has-special-flags, bits 13–0: a 14-bit address** — three independent pieces of state, not a single pointer with reserved values doing double duty:

- **Bit 15 — on/off.** A step with this bit set is lit and **always triggers the voice**, independent of anything else about the step. This is a deliberate decoupling from the address/lookup machinery below: whether a step fires is a single bit, checked once per tick, with nothing to look up.
- **Bit 14 — has special flags.** Set if the dynamic block at this step's address carries a special-flags byte (note/velocity/timing/roll/probability overrides, or the automation-hold flag). Clear if the step uses every default value for those fields.
- **Bits 13–0 — address**, with exactly **one** reserved value: `0x3FFF` (all 14 bits set) means **no lookup at all** — there is no dynamic block for this step, full stop. Combined with bit 15, that one reserved address covers both of the old "free" cases:
  - `0x3FFF` + on-bit clear → step is off, nothing stored (the old "blank").
  - `0x3FFF` + on-bit set → step is on, plays the track's default note/velocity/timing/probability (the old "default").
  - Any other address (`0x0000`–`0x3FFE`, 16,383 real values) → look up a dynamic block. Crucially, **the on-bit is independent of this** — a step can be *off* (doesn't trigger the voice) and still carry a real address with automation on it. That's new, and useful: it lets a parameter change land on a beat without also firing a trigger there, which wasn't possible when "has an address" and "the step is on" were the same fact.
...
- **If bit 14 (has-special) was set in the static entry**, byte 3 is **always** the special-flags byte, in a fixed position — not conditionally placed depending on the automation count. One bit per field: **note, velocity, timing, roll, probability** (5 currently defined — matching the step's own default values: default note, 100 velocity, standard timing, single trigger, 100% probability, each independently overridable) plus **automation hold** (a binary condition, needs no value byte of its own — see below) and 2 bits reserved for later. Each set *value* flag (the 5 named ones) contributes one override byte immediately after the special-flags byte, in flag order; `automation hold` contributes nothing since it's binary.
...
`automation hold` stays defined at the storage layer only as "a flag that exists" — what it does during playback (whether a held value persists until the next automation for that parameter anywhere later in the pattern, and whether that next automation becomes the new held value or a one-shot override) is `seq_tick()`-level playback logic for later in Phase 3, not a storage-format question.

**Worst case, using your own numbers:** a step using all 5 defined special fields and 4 automations costs `2 header bytes + 1 special-flags byte + 5 override bytes + 4×2 automation bytes = 16 bytes`. If every one of the 896 steps hit that simultaneously, the whole scene costs `896 × 16 = 14,336 bytes` — comfortably inside the 16,383-byte pool, with 2,047 bytes of headroom left over even in that extreme case, matching your own tally almost exactly. In the more realistic "every step has at least one automation, nothing else" case, the pool holds **7,295 total automation entries** — unchanged from the previous pass at this design, since the cheapest per-step case (a 2-byte header with no special-flags byte at all) costs the same either way.
...
Automation on a step plays back regardless of whether that step has a trigger — this is a change from the old velocity-0-as-automation-only-step model, and 3.2's final storage format makes it a literal, direct consequence of the design rather than a special case to handle: the on/off bit and the automation address are independent fields in the static entry, so a step's trigger state and its automation content were never coupled to begin with. Only steps with the on-bit set light their LED. Pressing a step button in step mode toggles that on-bit alone — it does **not** touch the step's address, note, velocity, or any other stored data, since none of that lives in the same bit. Additionally: holding `SHIFT+COPY/CLEAR` while the menu is up, then pressing sequence and `SELECT` buttons, clears just a step or just a bar of all automation/settings (distinct from toggling the on-bit off).
...
- **Advanced-buffer** — granular, drone, Karplus-Strong, convolution chamber. Only one of these can be active at a time system-wide, and they share a single **0.25-second, 16-bit, mono buffer in DTCM** (not ITCM — see below) for whichever one is currently in use.
...
- **`sine_table`** is read at true audio rate, per-sample, inside the oscillator block-calc functions (`Oscillator.c`), with a phase-accumulator index that's usually local/incrementing but can jump further per sample at high pitch — a somewhat less favorable pattern than `transientData`'s sequential one, and more like the pattern actual *code* execution from flash already relies on (which also works today, on the same ART cache). Reasonable confidence this is fine, but the specific case worth testing empirically is several simultaneous sine-based voices at widely different, high pitches, which is the scenario most likely to pressure the ART cache's working set. Worth an explicit stress test once the move is made, rather than assuming it's fine by analogy alone.
- Implementation is likely as simple as removing the `INCCM`/`INCCMZ` tag from both declarations — with no section attribute, `const` data falls through to the linker's default `.rodata` placement in app flash, the same place all the rest of the firmware's constant data and code already lives. No new linker section should be needed, but worth confirming the default `.rodata` target is genuinely the app-flash region (`0x08008000–0x0807FFFF`) and not anywhere near the runtime-erasable sample sectors (6–11), so a future sample write/erase can never touch them.
- Two smaller `INCCM`-tagged `const` tables exist too and are candidates for the same move if a little more headroom is wanted: `squareRootLut` (`Core/DSPAudio/squareRootLut.c`, 128 floats = 512 bytes) and `transientVolumeTable` (`Core/DSPAudio/transientTables.c`, 69 floats = 276 bytes) — both tiny, both read-only, both likely fine on the same reasoning as `transientData`.

**What's left in DTCM after the move, honestly:** the six voice structs (`DrumVoice[3]`, one each of `SnareVoice`/`CymbalVoice`/`HiHatVoice`), the six `ModulationNode` velocity modulators, and the mixer's per-voice state — all genuinely mutable, per-sample-updated DSP state, so none of it is a flash-relocation candidate; it has to stay in fast RAM. I can confirm these exist and are `INCCM`-tagged (so currently in DTCM), but each contains several nested sub-structs (`OscInfo`, `ResonantFilter`, `Lfo`, `TransientGenerator`, `Distortion`, `SlopeEg2`, `SnapEg`, and others), and hand-adding up their sizes would mean guessing at compiler padding/alignment — exactly the kind of false precision this document has been trying to avoid elsewhere. The real total needs the linker `.map` file from an actual build, not a manual tally. What I can say with confidence: it's real and non-trivial (six full voice structs is a lot more than the ~3.2KB I'd originally scoped), and the true "how much DTCM is left for the two new buffers" number depends on it directly.
...
A feedback path with a short delay/decay is also wanted. Originally I'd pointed this at the same buffer as the 6.7 FX-stack BBD delay; given 6.1's advanced-buffer tier now has its own dedicated 0.25-second/16-bit/mono buffer specifically for whichever of granular/drone/Karplus-Strong/convolution is active, it likely makes more sense for the granular voice's feedback/stacking behavior to live in *that* buffer rather than reaching into the separate, system-wide FX-stack delay — a voice's own feedback loop being self-contained in its own resource is cleaner than one voice type reaching into an unrelated subsystem's effect. Worth confirming which you intend, since it changes what "feedback" is actually doing here: local voice-level stacking (this buffer) versus routing through the shared FX delay (6.7's buffer, which every voice can already reach via the normal FX send).
...
- **Drone.** Interacting sub-oscillators, slow chaotic LFOs, internal bit-crush, a short delay/feedback loop (using the same 6.1 advanced-buffer-tier buffer as granular, since drone is also a member of that tier and only one of the four advanced-buffer voices runs at a time), integrated ring modulation, bit inversion, and some extra pre-wired or limited-selection internal LFOs (i.e., not the full general-purpose LFO routing the main voices get — a smaller, purpose-built set). No transient/envelope in the normal sense — it's meant to sit and drone once triggered.
...
**Memory — the honest picture, not the earlier undercount.** DTCM is 128KB total. Two DTCM-resident targets are worth pursuing:

- **BBD delay:** "1 second, or slightly more, whatever fits" (per your latest framing — a step up from the earlier "a little under 1 second is fine"), 8-bit, stereo. At the real sample rate (`REAL_FS ≈ 44,108Hz`), a full 1 second stereo at 1 byte/channel is `44,108 × 2 = 88,216 bytes (~86.15KB)`.
- **Advanced-buffer-tier buffer (6.1):** 0.25 second, 16-bit, mono — `44,108 × 0.25 × 2 = 22,054 bytes (~21.5KB)`.
- **Combined target: ~107.7KB**, out of 128KB total DTCM.

Whether that fits depends entirely on what's already resident, and 6.1 corrects a real undercount here: this section had originally claimed only ~3.2KB of DTCM was in use (`audioOutBuffer`/`audioOutBuffer2` plus two small oscillator interpolation buffers), based on grepping only the literal `INDTCM`/`INDTCMZ` macro names. That missed everything tagged via the `INCCM`/`INCCMZ` alias — which turns out to include `transientData` and `sine_table` (34,654 bytes, confirmed exactly, and the subject of 6.1's flash-relocation plan) *and* the six drum/snare/cymbal/hi-hat voice structs plus the six `ModulationNode` velocity modulators plus the mixer's per-voice state, none of which can move to flash since they're actively mutated every sample.

So the real arithmetic is: `128KB − (audio buffers + osc interp, ~3.2KB, staying) − (voice structs + modulators + mixer state, real but unmeasured) + (34,654 bytes freed by 6.1's flash move, if done) = headroom for the two new buffers`. The unmeasured term is the one that actually decides this, and it needs a real `.map` file to pin down — I'd rather say that plainly than repeat the earlier mistake of asserting a number the code doesn't actually support. What's genuinely good news: the 34.6KB the flash move frees is large relative to the ~107.7KB target, so it meaningfully improves the odds even without knowing the exact voice-struct total; whether it's enough depends on that measurement.
...
- **Full DTCM tally via a real `.map` file** — the single most important open item in this phase. 6.1/6.7 confirm two exact, freeable quantities (34,654 bytes in `transientData`+`sine_table`, plus 788 bytes more if `squareRootLut`/`transientVolumeTable` also move) and one large, real, but currently unmeasured quantity (six voice structs + six `ModulationNode`s + mixer state) standing between "128KB total" and "how much is actually left for the two new buffers." Nothing else in this phase's memory planning can be finalized with confidence until this number exists.
- **Granular/drone feedback buffer ownership (6.2/6.3)** — confirm whether the granular voice's feedback/stacking and the drone voice's delay/feedback loop should use their own dedicated 6.1 advanced-buffer-tier buffer (self-contained, only one active at a time) or route through the shared 6.7 FX-stack delay via the normal per-voice FX send (in which case "feedback" for these voices is really just "send a lot of this voice to the FX bus," not a separate buffer at all).
- **`sine_table`-in-flash polyphony stress test** — before committing, worth specifically testing several simultaneous sine-based voices at widely different, high pitches (the access pattern most likely to pressure the ART cache), rather than assuming it behaves like the lower-risk `transientData` case.
